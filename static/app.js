...
function requireLogin() {
  if (!getToken()) { location.href = '/login'; return true; }
  // 仪表盘为后台管理视图，仅管理员可访问；
  // 普通用户已登录但无仪表盘权限 → 跳转到服务端管理界面（/admin）浏览整合包/模组
  if (localStorage.getItem(ROLE_KEY) !== 'admin') { location.href = '/admin'; return true; }
  return false;
}
